...
**ユーザーからプロンプト（指示）を受け取るたびに、必ず以下を実行すること:**

1. `prompts/YYYY-MM-DD/AsheDism.md` に該当日のファイルがなければ作成する
2. ファイルが既にあれば末尾に追記する
3. 記録フォーマット:
   ```markdown
   ## N. 簡潔なタイトル

   > ユーザーの入力プロンプト（原文または要約）

   → 実施した対応の要約（1-3行）
   ```
4. 番号（N）は日付内で連番とする
5. **この処理はプロンプトを受け取った時点で必ず実行する。省略不可。**
...
- `gemini.ts` — Gemini 2.5 Flash 実装（フォールバック）
